...
VALUES (?, ?)"""
        ...
        stat.setInt(2, favorites.plan_id)
        stat.executeUpdate()
        stat.close()
        conn.close()
    }

    void delete(int user_id, int plan_id) {
        def conn = dataSource.getConnection()
        def sql = """
DELETE FROM likes WHERE user_id = ? AND plan_id = ?"""
        def stat = conn.prepareStatement(sql)
        stat.setInt(1, user_id)
        stat.setInt(2, plan_id)
        stat.executeUpdate()
        stat.close()
        conn.close()
    }

    Favorites get(int user_id, int plan_id) {
        def conn = dataSource.getConnection()
        def sql = """
SELECT user_id, plan_id FROM likes WHERE user_id = ? AND plan_id = ?"""
        def stat = conn.prepareStatement(sql)
        stat.setInt(1, user_id)
        stat.setInt(2, plan_id)
        ...
        favorites.plan_id = rs.getInt("plan_id")
        rs.close()
        stat.close()
        conn.close()
        favorites
...
FROM likes like, plans plan WHERE like.user_id = ? and like.user_id = plan.user_id and like.plan_id = plan.id """
        def stat = conn.prepareStatement(sql)
        stat.setInt(1, user_id)
        def rs = stat.executeQuery()
        List<Plan> list = []
